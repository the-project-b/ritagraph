...
export class ProjectBGraphQLClient {
  ...
  async requestWithFallback<T = any>(
    ...
    try {
      ...
      console.warn("GraphQL request failed, using fallback:", (error as Error).message);
  ...
  /**
   * Make a simple authenticated request with just a token
   */
  async requestWithToken<T = any>(
    document: RequestDocument,
    token: string,
    variables?: Variables
  ): Promise<T> {
