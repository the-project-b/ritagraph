...
if (process.env.LANGFUSE_PUBLIC_KEY && process.env.LANGFUSE_SECRET_KEY) {
  ...
  logger.info("LangFuse prompt repository initialized", {
    baseUrl: process.env.LANGFUSE_BASEURL || "https://cloud.langfuse.com",
    targetLabel: process.env.LANGFUSE_TARGET_LABEL,
    nodeEnv: process.env.NODE_ENV,
  });
} else {
  logger.debug("LangFuse credentials not provided, skipping initialization");
}

const defaultSource = process.env.PROMPT_SOURCE || "memory";
