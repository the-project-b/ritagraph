    ...
    if (isTemplateConfigured("template_initial_plan", config)) {
      ...
      const updatedState = {
        ...
        memory: safeCreateMemoryMap(state.memory)
          ...
          .set(
            "hasQuery",
            tasks.some((t) => t.type === "query")
          )
          .set(
            "hasMutation",
            tasks.some((t) => t.type === "mutation")
          ),
      };
      ...
      if (promptResult.messages && promptResult.messages.length > 0) {
        const planMessage =
          typeof promptResult.messages[0].content === "string"
            ? promptResult.messages[0].content.trim()
            : "";

        if (planMessage) {
          console.log(
            "🔧 INITIAL_PLAN - Generated template-based plan message:",
            planMessage
          );
          return planMessage;
        }
...
Generate ONLY the plan message, nothing else:`;
    ...
    console.warn(
      "🔧 INITIAL_PLAN - Both template and LLM failed, using hardcoded fallback:",
      error.message
    );
...
export const initialPlanNode = async (state: ExtendedState, config: any) => {
  ...
  try {
    ...
    if (!taskState || !userRequest) {
      throw new Error(
        "Missing task state or user request for initial plan generation"
      );
    }
    ...
    const planMessage = await generateInitialPlanMessage(
      userRequest,
      taskState.tasks,
      state,
      config
