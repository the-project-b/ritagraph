...
const createHumanReviewNode = () => {
  ...
  return async function humanReviewNode(
    ...
    const allowedActions = _config.configurable?.allowedActions || ['continue', 'update', 'feedback'];
    ...
    if (!allowedActions.includes(reviewAction as any)) {
      throw new Error(`Invalid review action: ${reviewAction}. Allowed actions: ${allowedActions.join(', ')}`);
    }
