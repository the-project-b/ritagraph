...
const validateGraphQLOperation = (query: string | undefined, variables?: string) => {
  if (!query) {
    return {
      isValid: false,
  ...
  try {
    ...
    if (missingVars.length > 0) {
      return {
        isValid: false,
        error: `Missing variable values for: ${missingVars.join(', ')}. Please provide matching variables.`
      };
    }
...
const createToolNode = (mcpTools: any[]) => {
  return async (
    ...
    for (const toolCall of lastMessage.tool_calls) {
      ...
      let toolResult = "";
      if (tool) {
        const toolArgs = { ...toolCall.args, accessToken: accessToken } as GraphQLToolArgs;
        try {
          // Validate GraphQL operations before execution
          if (toolCall.name === 'execute-query' || toolCall.name === 'execute-mutation') {
            if (!toolArgs.query) {
            ...
            const validation = validateGraphQLOperation(toolArgs.query, toolArgs.variables);
          ...
          if (toolCall.name === 'execute-query' || toolCall.name === 'execute-mutation') {
            if (e.message && (
              ...
              toolResult = `GraphQL Error: ${e.message}\n\n` +
                `Please ensure:\n` +
