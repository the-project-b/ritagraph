...
import { BuiltInQueryManager, builtInQueryManager, IntentMatch, BuiltInQueryHandler } from '../built-in-queries.tool';
...
describe('BuiltInQueryManager', () => {
  ...
  describe('executeHandler', () => {
    it('should execute existing handler successfully', async () => {
      const result = await manager.executeHandler('me', mockState, mockConfig, 'who am i');
    ...
    it('should return null for non-existent handler', async () => {
      const result = await manager.executeHandler('nonexistent', mockState, mockConfig, 'test');
  ...
  describe('handleBuiltInQuery', () => {
    it('should handle built-in query when handler exists', async () => {
      ...
      const result = await manager.handleBuiltInQuery(mockState, mockConfig, 'who am i', selectedQuery);
    ...
    it('should return null when no handler exists', async () => {
      ...
      const result = await manager.handleBuiltInQuery(mockState, mockConfig, 'test', selectedQuery);
  ...
  describe('me query handler', () => {
    it('should handle me query with valid task state', async () => {
      ...
      expect(task.queryDetails.queryResult.data.me.email).toBe('test@example.com');
    });
    ...
    it('should handle me query without task state', async () => {
      ...
      const result = await manager.executeHandler('me', stateWithoutTask, mockConfig, 'who am i');
  ...
  describe('version query handler', () => {
    it('should handle version query successfully', async () => {
      const result = await manager.executeHandler('version', mockState, mockConfig, 'what version');
      ...
      expect(task.queryDetails.queryResult.data.version.build).toBe('rita-multi-agent-v2.0.0');
    });
  });

  describe('health query handler', () => {
    it('should handle health query successfully', async () => {
      const result = await manager.executeHandler('health', mockState, mockConfig, 'system health');
  ...
  describe('error handling', () => {
    it('should handle missing task state gracefully', async () => {
      ...
      const result = await manager.executeHandler('me', stateWithoutTasks, mockConfig, 'who am i');
    ...
    it('should handle placeholder manager errors', async () => {
      const { placeholderManager } = require('../../../../placeholders/manager');
      placeholderManager.buildInvokeObject.mockRejectedValueOnce(new Error('Placeholder error'));
    ...
    it('should handle user service errors', async () => {
      const { userService } = require('../../../../utils/user-service.ts');
      userService.getUserName.mockRejectedValueOnce(new Error('User service error'));
