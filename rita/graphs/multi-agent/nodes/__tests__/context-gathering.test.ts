...
import { contextGatheringNode, GatheredContext } from '../context-gathering-node';
...
describe('Context Gathering Node', () => {
  ...
  const createMockTask = (id: string, queryDetails?: any): Task => {
    ...
    return {
      ...
      queryDetails: {
        ...
        rawTypeDetails: 'companyId: String!, contractIds: [String!]!, status: PaymentStatus'
      }
    } as Task;
  };
  ...
  describe('Basic Context Gathering', () => {
    ...
    test('should extract contract IDs from user request', async () => {
      const task = createMockTask('task_0');
      const state = createMockState('get payments for contracts id1, id2', [task]);
      ...
      expect(context.extractedPatterns.contractIds).toEqual(expect.arrayContaining(['id1', 'id2']));
    });
  ...
  describe('User Context Integration', () => {
    test('should extract user context from config and placeholders', async () => {
      ...
      jest.mocked(userService.getUserEmail).mockResolvedValue('config@example.com');
  ...
  describe('Dynamic Context from Previous Tasks', () => {
    test('should extract IDs from completed task results', async () => {
      ...
      const state = createMockState('get employee details', [currentTask], [completedTask]);
      
      const { context } = await runContextGathering(state);
      
      expect(context.dynamicContext.availableEmployeeIds).toEqual(['emp123', 'emp456']);
    ...
    test('should handle nested employee data structures', async () => {
      ...
      const currentTask = createMockTask('task_1');
      const state = createMockState('get more details', [currentTask], [completedTask]);
      ...
      expect(context.dynamicContext.employeeList).toHaveLength(2);
      expect(context.dynamicContext.availableEmployeeIds).toEqual(expect.arrayContaining(['emp123', 'emp456']));
    });
  });
  ...
  describe('Type-Aware Processing', () => {
    test('should generate resolution strategies based on type information', async () => {
      ...
      const companyIdStrategy = context.resolutionStrategies.find(s => s.parameter === 'companyId');
    ...
    test('should parse complex type definitions', async () => {
      const task = createMockTask('task_0', {
        ...
        rawTypeDetails: 'Required Fields:\n  companyId: String!\n  data: InputType!\nOptional Fields:\n  filter: FilterType'
      });
  ...
  describe('Context Gathering Node - Employee ID Handling', () => {
    ...
    test('Employee ID Resolution Strategy - Dynamic Context Detection', async () => {
      ...
      const state = createMockState('get contracts', [currentTask], [employeeListTask]);
      
      const { context: gatheredContext } = await runContextGathering(state);
      
      // Verify dynamic context has employee data
      expect(gatheredContext.dynamicContext.availableEmployeeIds).toContain('emp123');
      ...
      const employeeIdStrategy = gatheredContext.resolutionStrategies.find(s => s.parameter === 'employeeId');
    ...
    test('Employee ID Resolution Strategy - No Previous Employee Data', async () => {
      ...
      expect(gatheredContext.dynamicContext.availableEmployeeIds).toBeUndefined();
      ...
      expect(gatheredContext.contextAnalysis.missingRequiredParams).toContain('employeeId');
      expect(gatheredContext.contextAnalysis.workflowSuggestions).toHaveLength(1);
    ...
    test('Employee Company ID Resolution Strategy - Compound Parameter', async () => {
      ...
      const employeeCompanyIdStrategy = gatheredContext.resolutionStrategies.find(s => s.parameter === 'employeeCompanyId');
    ...
    test('Email-based Employee Query - Should Suggest Two-Task Workflow', async () => {
      ...
      const state = createMockState('get contract of employee with email emp_test@example.com', [currentTask]);
      
      const { context: gatheredContext } = await runContextGathering(state);
      
      // Verify that the email was extracted correctly
      expect(gatheredContext.staticContext.emails).toContain('emp_test@example.com');
      ...
      expect(suggestion.userMessage).toContain('none of the available employee queries support searching by email address');
      ...
      if (gatheredContext.staticContext.employeeIds) {
        expect(gatheredContext.staticContext.employeeIds).not.toContain('emp_test@example.com');
      }
    });
    ...
    test('Task Extraction - Email-based Employee Query Integration Test', async () => {
      ...
      const userRequest = 'get contract of employee with email emp_watsica-champlin_yjv2k@zfprmusw.mailosaur.net';
      ...
      expect(findEmployeeTask.description).toContain('emp_watsica-champlin_yjv2k@zfprmusw.mailosaur.net');
    ...
    test('Email-based Employee Query - Correct Query Selection', async () => {
      ...
      const findEmployeeTask = createMockTask('task_0', {
        ...
        rawTypeDetails: 'companyId: String!, status: [EmployeeAdvancedFilterStatus!]'
      });
      
      // Simulate the user request for finding employee by email
      const state = createMockState('find employee with email emp_test@example.com', [findEmployeeTask]);
      ...
      expect(gatheredContext.typeContext.optionalParameters).toContain('status');
      expect(gatheredContext.typeContext.requiredParameters).toContain('companyId');
      
      // Should not have unresolved parameters since we have companyId from user context
      const unresolvedRequired = gatheredContext.resolutionStrategies.filter(s => s.required && s.sources.length === 0);
      expect(unresolvedRequired).toHaveLength(0);
      
      // Should have resolution strategies for companyId
      const companyIdStrategy = gatheredContext.resolutionStrategies.find(s => s.parameter === 'companyId');
      expect(companyIdStrategy?.sources).toContain('user_context');
      
      // The status parameter should have default handling (if detected)
      const statusStrategy = gatheredContext.resolutionStrategies.find(s => s.parameter === 'status');
      if (statusStrategy) {
      ...
      console.log('🔍 Test Debug - Type Context:', {
        ...
        strategies: gatheredContext.resolutionStrategies.map(s => ({ param: s.parameter, sources: s.sources, confidence: s.confidence }))
      });
    });
    ...
    test('Two-Task Workflow - Second Task Context Inheritance', async () => {
      ...
      const state = createMockState('get contracts for employee', [getContractsTask], [findEmployeeTask]);
      ...
      expect(gatheredContext.contextAnalysis.missingRequiredParams).not.toContain('employeeId');
      
      // 4. Should NOT have workflow suggestions since employeeId is available
      expect(gatheredContext.contextAnalysis.workflowSuggestions).toHaveLength(0);
    });
    ...
    test('Two-Task Workflow - Real Failure Scenario', async () => {
      ...
      const findEmployeeTask = createTask({
        id: 'task_0',
        description: 'find employee with email emp_watsica-champlin_yjv2k@zfprmusw.mailosaur.net',
        type: 'query',
        targetAgent: 'query_agent',
        dependencies: [],
        status: 'completed',
      ...
      console.log('🔍 Real scenario dynamic context:', {
        availableEmployeeIds: gatheredContext.dynamicContext.availableEmployeeIds,
      ...
      if (gatheredContext.dynamicContext.availableEmployeeIds) {
        expect(gatheredContext.dynamicContext.availableEmployeeIds).toContain('user-uuid-123');
      } else {
        // This demonstrates the issue - no employeeIds extracted from complex structure
        console.log('❌ No employee IDs extracted from nested employee structure');
    ...
    test('Employee List Query - Multiple IDs and List Context', async () => {
      ...
      const listTask = createTask({
        ...
        result: {
          data: [
            { employeeId: 'emp123', name: 'John Doe', email: 'john@company.com' },
            { employeeId: 'emp456', name: 'Jane Smith', email: 'jane@company.com' }
          ]
        }
      });
      ...
      const currentTask = createMockTask('task_1');
      const listResultTaskState = createMockState('get details', [currentTask], [listTask]);
      
      const { context: gatheredContext } = await runContextGathering(listResultTaskState);
      
      // Verify list context
      expect(gatheredContext.dynamicContext.availableEmployeeIds).toContain('emp123');
      expect(gatheredContext.dynamicContext.availableEmployeeIds).toContain('emp456');
    ...
    test('Nested Employee Data - Complex Structure', async () => {
      ...
      const currentTask = createMockTask('task_1');
      const nestedResultTaskState = createMockState('get nested details', [currentTask], [nestedTask]);
      
      const { context: gatheredContext } = await runContextGathering(nestedResultTaskState);
    ...
    test('Multiple Employee Queries - Context Accumulation', async () => {
      ...
      const multiResultTaskState = createMockState('get multi details', [currentTask], [firstTask, secondTask]);
      
      const { context: gatheredContext } = await runContextGathering(multiResultTaskState);
      ...
      const mixedTask = createTask({
        ...
        result: {
          data: [
            ...
            { employee: { id: 'emp789' }, employeeId: 'emp789', name: 'Format 3' }
          ]
        }
      });

      const currentTask = createMockTask('task_1');
      const mixedResultTaskState = createMockState('get mixed details', [currentTask], [mixedTask]);
      
      const { context: gatheredContext } = await runContextGathering(mixedResultTaskState);
      
      expect(gatheredContext.dynamicContext.availableEmployeeIds).toContain('emp123');
      expect(gatheredContext.dynamicContext.availableEmployeeIds).toContain('emp456');
      expect(gatheredContext.dynamicContext.availableEmployeeIds).toContain('emp789');
    ...
    test('Empty Employee Results - Graceful Handling', async () => {
      ...
      const currentTask = createMockTask('task_1');
      const emptyResultTaskState = createMockState('get empty details', [currentTask], [emptyTask]);
      
      const { context: gatheredContext } = await runContextGathering(emptyResultTaskState);
    ...
    test('Two-Task Workflow - End-to-End Query Generation', async () => {
      ...
      const mockQuery = 'query { employeeWithContracts(data: { employeeId: {{employeeId}} }) { id } }';
      ...
      const resolvedQuery = mockQuery.replace('{{employeeId}}', `"${gatheredContext.dynamicContext.employeeId}"`);
      expect(resolvedQuery).toBe('query { employeeWithContracts(data: { employeeId: "emp123" }) { id } }');
    });
    ...
    test('INTEGRATION: Complete Email-based Employee Query Workflow', async () => {
      ...
      const state = createMockState(userRequest, [contractTask], [completedFindTask]);
      const { context: gatheredContext } = await runContextGathering(state);
      
      // Step 4: Verify context inheritance works correctly
      
      // Should extract email from original request
      expect(gatheredContext.staticContext.emails).toContain('emp_watsica-champlin_yjv2k@zfprmusw.mailosaur.net');
      ...
      expect(gatheredContext.contextAnalysis.hasAllRequiredParams).toBe(true);
      expect(gatheredContext.contextAnalysis.missingRequiredParams).toHaveLength(0);
      ...
      const mockQuery = 'query { employeeWithContracts(data: { employeeId: {{employeeId}}, employeeCompanyId: {{employeeCompanyId}} }) { id firstName lastName } }';
      ...
      const resolvedQuery = mockQuery
        .replace('{{employeeId}}', `"${gatheredContext.dynamicContext.employeeId}"`)
        .replace('{{employeeCompanyId}}', `"${gatheredContext.userContext.companyId}"`);
      
      expect(resolvedQuery).toBe('query { employeeWithContracts(data: { employeeId: "emp123", employeeCompanyId: "test-company" }) { id firstName lastName } }');
      
      console.log('✅ INTEGRATION TEST PASSED: Complete email-based employee query workflow verified');
    });
    ...
    test('Query Generation - Force Resolution of auto_companyid', async () => {
      ...
      const findEmployeeTask = createMockTask('task_0', {
        ...
        rawTypeDetails: 'companyId: String!, search: String, pagination: PaginationInputData',
        rawQueryDetails: 'employees(data: EmployeeAdvancedFilterForHrInput!): [OnboardingEmployee!]!'
      });
      ...
      const searchStrategy = gatheredContext.resolutionStrategies.find(s => s.parameter === 'search');
      ...
      console.log('✅ Query Generation Test: companyId resolution strategy verified');
      console.log('🔍 Available resolution strategies:', gatheredContext.resolutionStrategies.map(s => ({
    ...
    test('EmployeeBasicData Output Type - Contract Fields Inclusion', async () => {
      ...
      const employeesByCompanyTask = createMockTask('task_0', {
        ...
        rawTypeDetails: 'companyId: String!, status: [EmployeeAdvancedFilterStatus!]',
        rawQueryDetails: 'employeesByCompany(data: EmployeeAdvancedFilterForHrInput!): [EmployeeBasicData!]!',
        originalOutputType: 'EmployeeBasicData'
      });
      
      const state = createMockState('show me all employees', [employeesByCompanyTask]);
      ...
      expect(employeesByCompanyTask.queryDetails.originalOutputType).toBe('EmployeeBasicData');
      
      console.log('✅ EmployeeBasicData Test: Output type correctly detected');
      console.log('🔍 Task query details:', {
        selectedQueryName: employeesByCompanyTask.queryDetails.selectedQueryName,
        originalOutputType: employeesByCompanyTask.queryDetails.originalOutputType,
...
export const IntegrationTestScenarios = {
  ...
  simpleParameterExtraction: {
    ...
    userRequest: 'get payments for company acme with contracts emp1, emp2 and status active',
    expectedContext: {
      staticContext: {
        companyId: 'acme',
