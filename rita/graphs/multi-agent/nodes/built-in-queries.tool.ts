...
export interface BuiltInQueryHandler {
  name: string;
  description: string;
  handler: (state: ExtendedState, config: any, userRequest: string) => Promise<Command>;
}
...
export class BuiltInQueryManager {
  ...
  async handleBuiltInQuery(
    ...
    return await this.executeHandler(selectedQuery.name, state, config, userRequest);
  }
  ...
  private registerDefaultHandlers(): void {
    ...
    this.registerHandler({
      ...
      handler: async (state: ExtendedState, config: any, userRequest: string) => {
  ...
  private async handleMeQuery(
    ...
    if (currentTask) {
      ...
      try {
        invokeObject = await placeholderManager.buildInvokeObject('', { state: state as any, config });
        console.log('🔍 Built-in Query: Raw placeholder context:', invokeObject);
      } catch (error) {
        console.warn('🔍 Built-in Query: Placeholder system error:', error.message);
      }
      ...
      updatedTaskState.tasks[currentTaskIndex].queryDetails.queryResult = formattedResult;
