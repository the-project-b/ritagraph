...
function checkForUnresolvedPlaceholders(query: string): {
  ...
  const placeholders = placeholderMatches.map(match => match.slice(2, -2).trim());
...
export const queryExecutionNode = async (state: ExtendedState, config: any) => {
  ...
  try {
    ...
    if (!selectedQuery.generatedQuery) {
      throw new Error('No generated query found. Query generation node should run first.');
    }

    // Check for unresolved placeholders
    const placeholderCheck = checkForUnresolvedPlaceholders(selectedQuery.generatedQuery);
    ...
    if (placeholderCheck.hasUnresolved) {
      ...
      const errorMessage = `I need more information to complete your request. The following parameters are missing:\n\n` +
        placeholderCheck.placeholders.map((placeholder, index) => 
    ...
    const selectedQuery = currentTask.queryDetails;
    if (!selectedQuery) {
      throw new Error('ERROR: Processing execution query error. No selected query found.');
    }

    // Create more user-friendly error messages
    let userFriendlyError = error.message;
    if (error.message.includes('execute-query tool not found')) {
      userFriendlyError = 'The GraphQL execution service is currently unavailable. Please try again later.';
    } else if (error.message.includes('access') && error.message.includes('token')) {
      userFriendlyError = 'Authentication error. Please make sure you are properly logged in.';
    } else if (error.message.includes('syntax')) {
      userFriendlyError = 'There was an issue with the query structure. Please try rephrasing your request.';
