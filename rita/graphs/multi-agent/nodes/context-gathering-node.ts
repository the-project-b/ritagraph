...
export const ContextUtils = {
   ...
   */
  getMostRelevantContext(state: ExtendedState, currentTaskId?: string): GatheredContext | null {
...
function extractStaticParameters(
  ...
  const extractedPatterns = {
    ...
    dateRanges: [] as Array<{ startDate?: string; endDate?: string; type: string }>
  };
  ...
  employeePatterns.forEach(pattern => {
    ...
    matches.forEach(match => {
      if (match[2]) {
        ...
        const employeeIds = match[2].split(/[,\s]+/)
          .filter(id => id.trim())
          .filter(id => !id.includes('@')) // Exclude email addresses
          .filter(id => !['with', 'email', 'for', 'of', 'and'].includes(id.toLowerCase()));
        
        if (employeeIds.length > 0) {
          extractedPatterns.employeeIds.push(...employeeIds);
          staticContext.employeeIds = [...new Set(extractedPatterns.employeeIds)];
        }
      }
    });
  });
  ...
  if (requiredParams.includes('status') || parameterTypes.status) {
    ...
    const statusKeywords = ['active', 'pending', 'completed', 'cancelled', 'draft'];
  ...
  const hasDateParams = requiredParams.some(p => p.includes('date') || p.includes('Date')) ||
                        Object.keys(parameterTypes).some(p => p.includes('date') || p.includes('Date'));
  
  if (hasDateParams) {
    const specificDatePattern = /(?:from|since)\s+([0-9]{4}-[0-9]{2}-[0-9]{2})(?:\s+(?:to|until)\s+([0-9]{4}-[0-9]{2}-[0-9]{2}))?/gi;
        ...
        if (Array.isArray(result.data)) {
          ...
          result.data.forEach(item => {
            ...
            if (item.id) {
              ...
              } else if (item.firstName || item.lastName || item.email || item.role || item.employeeContract) {
                // This looks like an employee object - use id as employeeId
                employeeIds.add(item.id);
                console.log('🔍 Detected employee object with id:', item.id, 'fields:', Object.keys(item));
              }
            }
            
            // Check for nested employee data
            if (item.employee?.id) employeeIds.add(item.employee.id);
            if (item.employee?.employeeId) employeeIds.add(item.employee.employeeId);
          });
          ...
          if (result.data.employees && Array.isArray(result.data.employees)) {
            ...
            result.data.employees.forEach(emp => {
              ...
              if (emp.employee?.employeeId) employeeIds.add(emp.employee.employeeId);
            });
          }
          ...
          if (result.data.id) {
            if (result.data.firstName || result.data.lastName || result.data.email || result.data.role || result.data.employeeContract) {
              // This looks like an employee object - use id as employeeId
              employeeIds.add(result.data.id);
              console.log('🔍 Detected single employee object with id:', result.data.id, 'fields:', Object.keys(result.data));
            }
          }
          
          // Check for nested employee data
          if (result.data.employee?.id) employeeIds.add(result.data.employee.id);
          if (result.data.employee?.employeeId) employeeIds.add(result.data.employee.employeeId);
        }
      }
    }
    ...
    if (employeeIds.size > 0) {
      ...
      if (foundEmployeeList) {
        ...
        if (employeeListData.length > 0) {
          ...
          dynamicContext.employeeId = firstEmployee.employeeId || firstEmployee.id || firstEmployee.employee?.id;
          console.log('🔍 Setting default employeeId from list:', dynamicContext.employeeId);
        }
      } else {
 ...
 */
async function extractUserContext(state: ExtendedState, config: any): Promise<Record<string, any>> {
  const completedContext = getCompletedTasksContext(state);
  ...
  if (authUser) {
    ...
    if (authUser.email && !userContext.userEmail) userContext.userEmail = authUser.email;
    if (authUser.companyId && !userContext.companyId) userContext.companyId = authUser.companyId;
  }
  ...
  try {
    ...
    const { placeholderManager } = await import('../../../placeholders/manager');
    
    // Use placeholder system to get all user context data efficiently
    const placeholderContext = await placeholderManager.buildInvokeObject('', { state: state as any, config });
    ...
    for (const [placeholderKey, contextKey] of Object.entries(placeholderMapping)) {
      if (placeholderContext[placeholderKey] && !userContext[contextKey]) {
        ...
        if (contextKey === 'contractIds' && typeof value === 'string' && value.includes(',')) {
    ...
    const userFields = ['userId', 'userEmail', 'userRole', 'userLanguage', 'userDepartment'];
    ...
    console.log('🔍 Context Gathering: Raw placeholder context:', {
      ...
      resolvedFields: Object.keys(userContext).filter(key => userContext[key] !== undefined)
    });
    
  } catch (error) {
    console.warn('🔍 Context Gathering: Placeholder system error:', error.message);
    ...
    try {
      ...
      console.warn('🔍 Context Gathering: User service error:', fallbackError.message);
      ...
      try {
        ...
        if (authUser?.companyId) {
          ...
          console.log('🔍 Context Gathering: Using companyId from config fallback:', authUser.companyId);
        } else {
          console.warn('🔍 Context Gathering: No companyId available in config either:', {
        ...
        console.warn('🔍 Context Gathering: Config fallback error:', configError.message);
      }
    }
  }
  ...
  console.log('🔍 Context Gathering: Final user context:', {
    ...
    nonEmptyValues: Object.entries(userContext).filter(([k, v]) => v !== undefined && v !== null && v !== '')
  });
...
function extractTypeContext(currentTask: Task): GatheredContext['typeContext'] {
  ...
  if (selectedQuery) {
    ...
    if (selectedQuery.rawTypeDetails) {
      ...
      const requiredFieldsMatch = selectedQuery.rawTypeDetails.match(/Required Fields:\s*\n((?:\s*\w+:.*\n?)+)/);
      const optionalFieldsMatch = selectedQuery.rawTypeDetails.match(/Optional Fields:\s*\n((?:\s*\w+:.*\n?)+)/);
      ...
      if (requiredFieldsMatch) {
        ...
        while ((match = fieldPattern.exec(requiredFieldsMatch[1])) !== null) {
          ...
          if (paramName && paramName.length > 1 && !paramName.includes('Field') && !paramName.includes('Type')) {
      ...
      if (typeContext.requiredParameters.length === 0 && typeContext.optionalParameters.length === 0) {
        ...
        const fieldDefinitions = selectedQuery.rawTypeDetails.split(',').map(def => def.trim());
      ...
      if (typeContext.requiredParameters.length === 0 && typeContext.optionalParameters.length === 0) {
        ...
        const commonParams = ['companyId', 'pagination', 'conditionType', 'data', 'filter'];
        commonParams.forEach(param => {
          if (selectedQuery.rawTypeDetails.toLowerCase().includes(param.toLowerCase())) {
...
function generateResolutionStrategies(
...
): Array<{ parameter: string; sources: string[]; confidence: number; fallback?: string; required: boolean; type?: string }> {
  const strategies = [];
  
  // Create strategies for all known parameters (required + optional)
  const allParameters = [...typeContext.requiredParameters, ...typeContext.optionalParameters];
  ...
  for (const parameter of allParameters) {
    ...
    const dynamicKey = `available${parameter.charAt(0).toUpperCase() + parameter.slice(1)}`;
    const dynamicKeyPlural = `available${parameter.charAt(0).toUpperCase() + parameter.slice(1)}s`;
    
    // Check for array of available values (e.g., availableEmployeeIds)
    if (dynamicContext[dynamicKeyPlural] && Array.isArray(dynamicContext[dynamicKeyPlural]) && dynamicContext[dynamicKeyPlural].length > 0) {
      sources.push('dynamic_context');
      confidence = Math.max(confidence, 0.6);
    }
    // Check for singular array (e.g., availableEmployeeId)
    else if (dynamicContext[dynamicKey] && Array.isArray(dynamicContext[dynamicKey]) && dynamicContext[dynamicKey].length > 0) {
      sources.push('dynamic_context');
      confidence = Math.max(confidence, 0.6);
    }
    ...
    if (parameter === 'contractIds' || parameter === 'contractId') {
      if (staticContext.contractIds?.length > 0) {
        ...
        sources.push('dynamic_context');
        confidence = Math.max(confidence, 0.6);
      } else if (userContext.contractIds && (Array.isArray(userContext.contractIds) ? userContext.contractIds.length > 0 : userContext.contractIds)) {
    ...
    if (parameter === 'search') {
      ...
      else if (staticContext.employeeIds && staticContext.employeeIds.length > 0) {
        sources.push('static_request');
        confidence = Math.max(confidence, 0.8);
      }
      // Check if we have dynamic context that could be used for search
      else if (dynamicContext.availableEmployeeIds && dynamicContext.availableEmployeeIds.length > 0) {
        sources.push('dynamic_context');
        confidence = Math.max(confidence, 0.6);
      }
    }
    ...
    if (parameter === 'status') {
      ...
      else if (typeContext.requiredParameters.includes('status') || typeContext.optionalParameters.includes('status')) {
  ...
  return strategies.filter(strategy => strategy.sources.length > 0 || strategy.required);
}

/**
 * Context Gathering Node - Main function
 */
export const contextGatheringNode = async (state: ExtendedState, config: any) => {
  ...
  try {
    ...
    const unresolvedRequired = resolutionStrategies.filter(s => s.required && s.sources.length === 0);
    const contextAnalysis = {
      hasAllRequiredParams: unresolvedRequired.length === 0,
      missingRequiredParams: unresolvedRequired.map(s => s.parameter),
      availableDataTypes: Object.keys(dynamicContext).filter(key => 
        key.startsWith('available') || key.endsWith('List') || key.endsWith('Id')
      ),
      workflowSuggestions: []
    };
    ...
    if (unresolvedRequired.length > 0) {
      for (const missing of unresolvedRequired) {
        if (missing.parameter === 'employeeId' && !dynamicContext.availableEmployeeIds) {
          ...
          if (staticContext.emails && staticContext.emails.length > 0) {
            contextAnalysis.workflowSuggestions.push({
              ...
              action: 'Email-based employee search is not supported by available queries',
            ...
            contextAnalysis.workflowSuggestions.push({
              ...
              userMessage: 'I need employee information first. Let me get the employee list for you.'
            });
          }
        } else if (missing.parameter === 'contractId' && !dynamicContext.availableContractIds) {
          contextAnalysis.workflowSuggestions.push({
            missing: 'contractId',
            suggestion: 'prerequisite_query',
            action: 'Run contract list query first',
            queryType: 'contract_list',
            userMessage: 'I need contract information first. Let me get the contract list for you.'
          });
        }
      }
    }
    ...
    const contextHistory = (updatedMemory.get('contextHistory') as ContextHistoryEntry[]) || [];
    ...
    logEvent('info', AgentType.TOOL, 'context_gathering_completed', {
      ...
      unresolved: resolutionStrategies.filter(s => s.sources.length === 0 && s.required).map(s => s.parameter),
      extractedPatterns: {
    ...
    const nextNode = currentTask?.type === 'mutation' ? "MUTATION_GENERATION" : "QUERY_GENERATION";
