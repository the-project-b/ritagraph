...
function hasCycle(tasks: Task[]): { hasCycle: boolean; cyclePath?: string[] } {
  ...
  function dfs(taskId: string, path: string[] = []): { hasCycle: boolean; cyclePath?: string[] } {
...
export function getNextTask(state: ExtendedState): { task: Task | null; updatedState: ExtendedState } {
  ...
  if (cycleCheck.hasCycle) {
    console.error(`🚫 TASK SELECTOR - Detected cyclic dependency: ${cycleCheck.cyclePath?.join(' → ')}`);
    ...
    const updatedTaskState = {
      ...taskState,
      tasks: updatedTasks,
      failedTasks: new Set([...taskState.failedTasks, ...(cycleCheck.cyclePath || [])])
    };
...
const TaskInterfaceSchema = z.object({
  ...
  context: z.object({
    ...
    phase: z.enum(['initialization', 'data_gathering', 'execution', 'completion']),
...
export function needsVerification(task: Task): boolean {
  ...
  if (task.citations.some(citation => citation.verificationStatus === 'needs_verification')) {
    return true;
  }
...
For simple requests like "who am I", create a single query task.`;
  ...
  try {
    ...
    if (!response || !response.content) {
      console.warn('LLM returned empty response, falling back to basic extraction');
    ...
    if (!content) {
      console.warn('LLM response is empty after trimming, falling back to basic extraction');
    ...
    if (startIndex === -1) {
      console.warn('No JSON array found in LLM response, falling back to basic extraction');
    ...
    if (endIndex === -1) {
      console.warn('No closing bracket found in LLM response, falling back to basic extraction');
    ...
    return validationResult.data.map((task, index) => createTask({
      id: `task_${index}`,
...
const basicExtractTasks = (request: string): Task[] => {
  ...
  for (let i = 0; i < parts.length; i++) {
    ...
    if (!part || ['and', 'then', 'after', 'next', 'before', 'while', 'when'].includes(part.toLowerCase())) {
      continue;
    }
    
    // Determine task type based on keywords
    const isMutation = /\b(create|update|delete|modify|change|set|add|remove|edit|rename)\b/i.test(part);
...
export function calculateTaskConfidence(task: Task): number {
  ...
  const sourceConfidence = task.sources.length > 0
    ? task.sources.reduce((sum, source) => sum + source.confidence, 0) / task.sources.length
    : 0;

  // Calculate citation confidence (60% weight)
  // Give higher weight to verified citations
  const citationConfidence = task.citations.length > 0
    ? task.citations.reduce((sum, citation) => {
        const citationWeight = citation.verificationStatus === 'verified' ? 1.2 : 1.0;
...
export function extendTaskStateWithNewTasks(
  state: ExtendedState, 
  { newTasks, executionStartTime }: { newTasks: Task[]; executionStartTime?: number }
): ExtendedState {
  ...
  const extendedTaskState: TaskState = {
    ...
    executionStartTime: executionStartTime || existingTaskState.executionStartTime
  };
...
export function resetTaskState(
  ...
  return {
    ...
    executionStartTime: (resetCompleted && resetFailed && resetInProgress) 
      ? Date.now() 
...
export function getCompletedTasksContext(state: ExtendedState): {
  ...
  let userInfo = null;
  const userInfoTask = completedTasks.find(task => 
  ...
  completedTasks.forEach(task => {
    if (task.result && task.result.data) {
      ...
      const contextKey = task.description.toLowerCase()
        .replace(/[^a-z0-9\s]/g, '')
        .replace(/\s+/g, '_');
...
export function clearCompletedTaskHistory(state: ExtendedState): ExtendedState {
  ...
  const activeTasks = taskState.tasks.filter(task => 
