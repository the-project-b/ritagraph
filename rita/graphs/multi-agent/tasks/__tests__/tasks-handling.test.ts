...
describe('TasksHandling - Core Fixes', () => {
    ...
    it('should detect task references in memory when no taskState exists', () => {
      ...
      const stateWithTaskReferences: ExtendedState = {
        ...
        memory: new Map<string, any>([
          ...
          ['queryContext', JSON.stringify({ relatedTo: 'task_2', dependsOn: 'task_1' })],
      ...
      expect(result.memory?.get('lastResult')).toBe('Based on task_3 result, user is admin');
    });
  });
  ...
  describe('getNextTask - Task Selection Logic', () => {
    it('should select task with all dependencies completed using actual task status', () => {
      ...
      const updatedTaskState = updatedState.memory?.get('taskState') as TaskState;
      const selectedTask = updatedTaskState.tasks.find(t => t.id === 'task_1');
  ...
  describe('Context Preservation', () => {
    it('should preserve completed task context for future reference', () => {
      ...
      expect(context.recentResults[0]).toEqual({ data: 'John Doe', role: 'admin' });
      expect(context.recentResults[1]).toEqual({ permissions: ['read', 'write', 'admin'] });
  ...
  describe('Task Progress and State Management', () => {
    ...
    it('should handle task progress updates correctly', () => {
      ...
      const successTaskState = successResult.memory?.get('taskState') as TaskState;
      ...
      const failureTaskState = failureResult.memory?.get('taskState') as TaskState;
