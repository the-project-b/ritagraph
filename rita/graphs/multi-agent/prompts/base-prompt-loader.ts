...
export abstract class BasePromptLoader {
  ...
  protected extractTemplateStrings(promptTemplate: any): string {
    ...
    if (promptTemplate.lc_kwargs?.messages && Array.isArray(promptTemplate.lc_kwargs.messages)) {
    ...
    console.log('🔧 Template extraction result:', {
      ...
      preview: combinedTemplateString.substring(0, 100) + (combinedTemplateString.length > 100 ? '...' : '')
    });
  ...
  protected async buildDynamicInvokeObject(
    ...
    if (!Array.isArray(inputVariables)) {
      console.warn('Input variables is not an array, defaulting to empty array:', inputVariables);
      inputVariables = [];
    }
    ...
    try {
      console.log('🔧 About to call placeholderManager.buildInvokeObjectWithRequiredVars with:', {
      ...
      if (typeof placeholderManager.buildInvokeObjectWithRequiredVars === 'function') {
        ...
        console.warn('🔧 buildInvokeObjectWithRequiredVars not available, falling back to buildInvokeObject');
        const result = await placeholderManager.buildInvokeObject(combinedTemplateString, compatibleContext);
