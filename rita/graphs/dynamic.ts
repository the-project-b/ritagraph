...
const create_dynamic_graph = async () => {
  try {
    ...
    const dynamicPromptEntryNode = async (
      ...
      const promptId = config?.configurable?.prompt_ids?.[0] || config?.configurable?.promptId;
      
      if (!promptId) {
        throw new Error("promptId is required in configurable.prompt_ids[0] or configurable.promptId");
      }
